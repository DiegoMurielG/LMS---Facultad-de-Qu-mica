...
export default function RegistrarSeccion({ id_curso }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sections_curso_response = await api.post("/buscar-cursos", {
      ...
      const response = await api.post("/registrar-seccion", {
      ...
      if (response.data.Status === 405) {
        ...
        const promesaActualizarCurso = [id_seccion].map(async (id_seccion_obtenido) => {
          return await api.post("/actualizar-secciones-curso", {
            id_curso: id_curso,
        ...
        const promesasActividades = actividadesSeleccionadas.map(async (actividad) => {
          const actividadResponse = await api.post("/buscar-actividades", {
          ...
          await api.post("/actualizar-idSection-actividad", {
